...
@HEADS.register_module
class ClsRecMixupHead(BaseModule):
    """Mixup Classifier Head, with only one fc layer.
    Args:
        with_avg_pool (bool): Whether to use GAP before this head.
        loss (dict): Config of classification loss.
    ...
    def __init__(self,
                 ...
                 frozen=False,
                 init_cfg=None):
        super(ClsRecMixupHead, self).__init__(init_cfg=init_cfg)
        self.with_avg_pool = bool(with_avg_pool)
        self.in_channels = int(in_channels)
        self.num_classes = int(num_classes)
        self.share_fc = bool(share_fc)
        ...
        # fc layer
        self.fc = nn.Linear(in_channels, num_classes)
        self.fc_roi = nn.Linear(in_channels, num_classes)
        if frozen:
            self._freeze()
        # post-process for inference
        post_process = getattr(self.criterion, "post_process", "none")
    ...
    def init_weights(self, init_linear='normal', std=0.01, bias=0.):
        if self.init_cfg is not None:
            super(ClsRecMixupHead, self).init_weights()
            return
        assert init_linear in ['normal', 'kaiming', 'trunc_normal'], \
            "Undefined init_linear: {}".format(init_linear)
        for m in self.modules():
    ...
    def forward(self, x, sz=32, boxes=None, post_process=None, **kwargs):
        """Inference.
        ...
        # simple test
            ...
            if boxes is not None:
                index = torch.arange(x.size(0)).view(-1, 1).to(x.device)
                boxes = torch.cat([index, boxes], 1)
                spatial_scale = x.shape[-1] / sz
                roi_feat = roi_align(x, boxes,
                                     output_size=(1, 1),
    ...
    def loss(self, cls_score, labels, label_mask=None, **kwargs):
        ...
        if not isinstance(labels, tuple):
            ...
            target = labels.clone()
            # default onehot cls
            losses['loss'] = self.criterion(cls_score, target, avg_factor=avg_factor, **kwargs)
            # compute accuracy
            losses['acc'] = accuracy(cls_score, labels)
        # 2. mixup classification
        else:
            ...
            # 2.1 mixup (hard ce) cls (using softmax)
            losses['loss'] = self.criterion(cls_score, y_a, avg_factor=avg_factor, **kwargs) * lam + \
                             self.criterion(cls_score, y_b, avg_factor=avg_factor, **kwargs) * (1 - lam)
...
class ClsUncertainMixupHead(BaseModule):
    ...
    def __init__(self,
        ...
        # fc layer
        self.fc = nn.Linear(in_channels, num_classes)
        self.sematic = nn.Linear(num_classes, num_classes)
        self.uncentain = nn.Linear(num_classes, num_classes)
    ...
    def l2_norm(self, x):
        ...
        norm = torch.sqrt(torch.sum(buffer, 1).add_(1e-12))
        output = torch.div(x, norm).view(x_size)
    ...
    def forward_head(self, x):
        ...
        x = self.fc(x)   # [100,512] -> [100,100]  cls-head
        out = self.post_process(x)   # softmax to (0,1)
        if self.is_norm:
            x_semantic = self.softmax(self.sematic(out))
            x_uncentain = self.softmax(self.uncentain(out))

        return x, x_semantic, x_uncentain
    ...
    def INloss(self, cls_score, labels, **kwargs):
        r"""" mixup classification loss forward
        if mixup:
            (ya, yb, lam) = labels
            cls_score(
                (x, x_sematic, x_uncentaion) = cls_original
                (x1, x1_sematic, x_uncentain) = cls_mixed
                index
                    )
        else:
        ...
        y_a, y_b, lam = labels
        img_out, img_s, img_un = cls_score[0]
        mix_out, mix_s, mix_un = cls_score[1]
        index = cls_score[-1]
        if isinstance(lam, torch.Tensor):  # lam is scalar or tensor [N,\*]
            lam = lam.view(-1, 1)
        avg_factor = y_a.size(0)

        # beta : uncertain; alpha : semantic; eps : gussain distribution
        sq_img_s, sq_mix_s = self.l2_norm(img_s), self.l2_norm(mix_s)
        sq_img_un, sq_mix_un = self.l2_norm(img_un), self.l2_norm(mix_un)
        eps = torch.rand_like(sq_img_s)

        beta_a = sq_img_un + sq_mix_un
        alpha_a = sq_img_s + sq_mix_s - 2*torch.mm(img_s, self.softmax(mix_out))
        IN_a = alpha_a * torch.exp((-1.0) * torch.div(beta_a, alpha_a))
        # print("eps")
        # print(eps)
        # print("alpha & beta")
        # print(alpha_a, beta_a)

        beta_b = sq_img_un[index] + sq_mix_un
        alpha_b = sq_img_s[index] + sq_mix_s - 2*torch.mm(img_s[index], self.softmax(mix_out))
        IN_b = alpha_b * torch.exp((-1.0) * torch.div(beta_b, alpha_b))

        # print(IN_a.shape, IN_b.shape)
        # print("IN")
        # print(IN_a, IN_b)

        # 2.1 mixup (hard ce) cls (using softmax)
        # losses['loss'] = (1 - self.gama) * (self.criterion(mix_out * IN_a, y_a, avg_factor=avg_factor, **kwargs) * lam +
        #                                     self.criterion(mix_out * IN_b, y_b, avg_factor=avg_factor, **kwargs) * (1 - lam))+ \
        #                  self.gama * self.criterion(img_out, y_a, avg_factor=avg_factor, **kwargs)
        losses['loss'] = (1 - self.gama) * (self.criterion(mix_out, y_a, avg_factor=avg_factor, **kwargs) * lam +
                                            self.criterion(mix_out, y_b, avg_factor=avg_factor, **kwargs) * (1 - lam)) + \
                         self.gama * (self.criterion(IN_a, y_a, avg_factor=avg_factor, **kwargs) * lam +
                                            self.criterion(IN_b, y_b, avg_factor=avg_factor, **kwargs) * (1 - lam))
        # compute accuracy
        losses['acc'] = accuracy(mix_out, labels[0])
        losses['acc_mix'] = accuracy_mixup(mix_out, labels)
        return losses
